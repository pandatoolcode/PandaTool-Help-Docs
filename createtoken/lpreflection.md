...
注2：不是自己加池子获得的LP，必须交易一笔才能激活，否则不能获得分红，建议不要转账LP
...
* [x] **买入税率** (不需要的部分不能填空，必须填0/营销税除外，总比例小于25%):
...
* [x] **交易控制**
  ...
  * ~~**添加交易对：**&#x5982;果您的分红代币在不同的交易所上架, 或存在不同的交易对, 请将对应的流动池地址添加进来, 否则税率、分红等机制将在其他交易对上不起作用~~**（该功能已失效）**
* [x] **税率控制**
